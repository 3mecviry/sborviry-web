...
(function () {
      ...
      <header class="header" id="hdr">
        <div class="wrap header-inner">
          ...
          <div class="header-actions">
            ...
            <a class="btn btn-primary btn-sm header-cta" href="${href('first')}">${esc(t('cta.first'))}</a>
  ...
  function formatDate(iso) {
    const map = { cs: 'cs-CZ', pl: 'pl-PL', en: 'en-GB', es: 'es-ES' };
    try {
      return new Date(iso + 'T00:00:00').toLocaleDateString(map[LANG] || 'cs-CZ',
  ...
  function renderYouTube() {
    const host = $('#yt');
    if (!host) return;
    const id = DATA.youtubeChannelId;
    if (id) {
      host.innerHTML = `<div class="media ratio-16-9">
        <iframe src="https://www.youtube.com/embed/live_stream?channel=${encodeURIComponent(id)}"
          title="Sbor Víry — YouTube" loading="lazy" allowfullscreen
  ...
  function renderGiving() {
    ...
    host.innerHTML = `<div class="times">
      ${rows.map((r) => `<div class="time-row">
        ...
        <span style="font-family:'Bricolage Grotesque',sans-serif;font-size:1.15rem;color:var(--ink);letter-spacing:-.01em">${esc(r[1])}</span>
      </div>`).join('')}
  ...
  function renderGallery() {
    ...
    // položka může být buď cesta ('assets/img/x.jpg'), nebo { src, alt: { cs, pl, … } }
    $('#gallery').innerHTML = list.map((item, i) => {
      const src = typeof item === 'string' ? item : item.src;
      const alt = typeof item === 'string' ? t('gallery.alt') : (loc(item.alt) || t('gallery.alt'));
      return `<figure class="media ${i === 0 ? 'ratio-4-3' : 'ratio-1-1'}"
                      data-photo="${esc(src)}" data-photo-alt="${esc(alt)}"></figure>`;
    }).join('');
  }
