...
(function () {
  ...
  const DATE_LOCALE = {
    cs: 'cs-CZ', sk: 'sk-SK', pl: 'pl-PL', uk: 'uk-UA', ru: 'ru-RU',
  ...
  function fitNav() {
    ...
    nextFrame(() => {
      ...
      const vejdeSe = () => inner.scrollWidth <= inner.clientWidth + 1 &&
                            (!nav || nav.scrollWidth <= nav.clientWidth + 1);

      // Ustupuje se ve dvou krocích. Nejdřív zmizí tlačítko „Jsem tu poprvé“ —
      // stejný odkaz je i v menu, takže se nic neztratí a položky zůstanou
      // vidět na užších displejích. Teprve když nestačí ani to, sbalí se celé
      // menu do tlačítka. (Čtení scrollWidth si samo vynutí přepočet rozvržení,
      // druhé měření tedy už vidí hlavičku bez tlačítka.)
      let fits = vejdeSe();
      if (!fits) {
  ...
  function renderGiving() {
    ...
    // Vlastní třídy, ne .times — tabulka časů má jiné rozvržení a dřív si
    // je tyhle dvě tabulky sdílely, takže se úprava jedné projevila i ve druhé.
    host.innerHTML = `<div class="data-rows">
      ${rows.map((r) => `<div class="data-row">
  ...
  function renderSchema() {
    ...
    /* Setkání se záměrně neznačí jako události (Event). Google u události
       vyžaduje konkrétní datum konání (startDate) — na statickém webu by
       zastaralo — a samotný týdenní rozvrh hlásila Search Console jako
       chybu (srpen 2026). Kdy a kde se sbor schází, říkají otevírací doba
       výš a text stránek. */
